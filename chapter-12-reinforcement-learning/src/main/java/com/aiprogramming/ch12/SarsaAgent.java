...
public class SarsaAgent extends Agent {
    ...
    public void learn(int state, int action, double reward, int nextState, boolean done) {
        // This is a simplified learn method for SARSA
        // In practice, SARSA needs the next action as well
        // This version assumes the next action is chosen greedily
        
        double currentQValue = qTable[state][action];
        double nextQValue = done ? 0.0 : qTable[nextState][getGreedyAction(nextState)];
