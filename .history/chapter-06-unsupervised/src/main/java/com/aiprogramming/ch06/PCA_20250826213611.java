...
public class PCA {
    ...
    public void fit(List<DataPoint> dataPoints) {
        ...
        RealVector eigenvalues = eigenDecomp.getRealEigenvalues();
        RealMatrix eigenvectors = eigenDecomp.getV();
        
        // Sort eigenvalues and eigenvectors in descending order
        int[] sortedIndices = getSortedIndices(eigenvalues.toArray());
        
        // Extract top nComponents eigenvectors
        components = new Array2DRowRealMatrix(nComponents, nFeatures);
        for (int i = 0; i < nComponents; i++) {
            int idx = sortedIndices[i];
            components.setRowVector(i, eigenvectors.getColumnVector(idx));
        }
    ...
    public List<DataPoint> transform(List<DataPoint> dataPoints) {
        ...
        for (DataPoint point : dataPoints) {
            ...
            RealVector projected = components.multiply(centeredPoint);
    ...
    public List<DataPoint> inverseTransform(List<DataPoint> dataPoints) {
        ...
        for (DataPoint point : dataPoints) {
            ...
            // Inverse transform
            RealVector originalPoint = components.transpose().multiply(projectedPoint);
     ...
     */
    private int[] getSortedIndices(RealVector values) {
        ...
        Arrays.sort(indices, (a, b) -> Double.compare(values.getEntry(b), values.getEntry(a)));
