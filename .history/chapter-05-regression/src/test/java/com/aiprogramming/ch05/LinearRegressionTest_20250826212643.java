...
public class LinearRegressionTest {
    ...
    void setUp() {
        ...
        // Multiple features data: y = 2x1 + 3x2 + 1 (independent features)
        multipleData = new ArrayList<>();
        Random random = new Random(42); // Fixed seed for reproducibility
        for (int i = 1; i <= 10; i++) {
            Map<String, Double> features = new HashMap<>();
            features.put("x1", (double) i);
            features.put("x2", random.nextDouble() * 10); // Independent random values
            double target = 2.0 * i + 3.0 * features.get("x2") + 1.0;
    ...
    void testMultipleLinearRegression() {
        ...
        // Test that coefficients are reasonable (not NaN or infinite)
        Map<String, Double> coefficients = regression.getCoefficients();
        assertFalse(Double.isNaN(coefficients.get("x1")));
        assertFalse(Double.isNaN(coefficients.get("x2")));
        assertFalse(Double.isInfinite(coefficients.get("x1")));
        assertFalse(Double.isInfinite(coefficients.get("x2")));
        
        // Test that intercept is reasonable
        assertFalse(Double.isNaN(regression.getIntercept()));
        assertFalse(Double.isInfinite(regression.getIntercept()));
        
        // Test prediction with known values
        Map<String, Double> testFeatures = new HashMap<>();
        testFeatures.put("x1", 5.0);
        testFeatures.put("x2", 2.0);
        double prediction = regression.predict(testFeatures);
        
        // Prediction should be reasonable (not NaN or infinite)
        assertFalse(Double.isNaN(prediction));
        assertFalse(Double.isInfinite(prediction));
        assertTrue(prediction > 0); // Should be positive given our data
    }
